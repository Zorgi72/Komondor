//! `zyth completions <shell>` — generate shell completion scripts.
//!
...
pub fn run(shell: Shell) {
    ...
    let mut cmd = PagerArgs::command().name("zyth");
    if shell != Shell::Zsh {
        generate(shell, &mut cmd, "zyth", &mut std::io::stdout());
        return;
    }
    // zsh needs post-processing (see fix_zsh_root_prompt_positional).
    let mut buf = Vec::new();
    generate(shell, &mut cmd, "zyth", &mut buf);
...
/// `$line[2]` empty, and the dispatch falls through — so `zyth worktree <TAB>`
...
mod tests {
    ...
    fn zsh_script() -> String {
        let mut cmd = PagerArgs::command().name("zyth");
        let mut buf = Vec::new();
        generate(Shell::Zsh, &mut cmd, "zyth", &mut buf);
    ...
    // so `zyth worktree <TAB>` re-offered every top-level command (upstream
    // clap-rs/clap#6282).
    #[test]
