//! Headless single-turn mode (`zyth -p "prompt"`).
//!
...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `zyth login` to authenticate \
         (or `zyth login --device-code` if no browser is available)."
            .to_string()
    } else {
        "Not signed in. To authenticate without a browser, run:\n  \
         grok login --device-code\n\n\
         Alternatively, set the XAI_API_KEY environment variable \
         or run `zyth login` on a machine with a browser."
            .to_string()
    }
}
...
pub async fn run_single_turn(
    ...
    // hub_url lives on `zyth workspace` / WorkspaceStartArgs only).
