...
//! system clipboard. This is the engine behind `zyth wrap` (see
...
///
/// This is the engine behind `zyth wrap`: it spawns
...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // `zyth` reached over SSH — that its OSC 52 clipboard writes are being
    ...
    // detached — `zyth wrap` is short-lived and exits with the child.
