...
/// `zyth "do the thing"` does not re-submit on resume. Keeps everything else
...
pub(crate) fn build_screen_mode_relaunch_args(
    ...
    while let Some(arg) = iter.next() {
        ...
        // Bare positional prompt (e.g. `zyth "fix the bug"`). Must not re-fire
...
/// as a successful `exec` — plain `zyth --resume <id>` (or even
/// `zyth --minimal --resume <id>`) is not enough after `/fullscreen` when
...
pub(crate) fn screen_mode_relaunch_resume_hint(session_id: &str, want_minimal: bool) -> String {
    ...
    format!("{GROK_SCREEN_MODE_ENV}={mode} zyth {flag} --resume {session_id}")
}
...
/// (tool shells, workers, nested `zyth` invocations) would inherit a forced
...
mod tests {
    ...
    fn double_dash_and_following_positionals_dropped() {
        // `zyth --no-leader -- "fix the bug"`: everything after `--` is the
    ...
    fn resume_without_value_then_flag_is_not_eaten() {
        // `zyth --resume --no-leader` (resume most-recent; next token is a flag).
        let out = build_screen_mode_relaunch_args(
    ...
    fn failed_relaunch_hint_includes_screen_mode_env() {
        ...
        assert_eq!(
            ...
            "GROK_SCREEN_MODE=fullscreen zyth --fullscreen --resume abc-sid"
        );
        assert_eq!(
            screen_mode_relaunch_resume_hint("abc-sid", true),
            "GROK_SCREEN_MODE=minimal zyth --minimal --resume abc-sid"
        );
    }
