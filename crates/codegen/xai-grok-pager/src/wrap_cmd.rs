//! `zyth wrap` — run any command in a local PTY that forwards its clipboard.
//!
//! Generalizes the `zyth ssh` wrapper: spawns an arbitrary command inside a
...
//! run — a single shell-quoted string (`zyth wrap "mycli ssh host"`) or a shell
    ...
    if should_wrap() {
        match crate::pty_wrap::run_wrapped_command(&wrapped.program, &wrapped.args) {
            ...
            Err(e) => {
                ...
                eprintln!("zyth wrap: wrapped mode failed, running without PTY wrapping: {e}");
...
/// The program and argv `zyth wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
...
fn derive_spawn(
    ...
    // (`zyth wrap "mycli ssh host"`), not a program name: hand it to the shell
    ...
    // name, and an empty one (`zyth wrap "$PROG" ...` with `$PROG` unset)
...
///
/// Unlike `zyth ssh`, `zyth wrap` does not gate on the terminal brand: the user
