//! Host clipboard image paste mediated by `zyth wrap`.
//!
...
//! the inner `zyth`) can solicit the host pasteboard. That is intentional and
//! acceptable for `zyth wrap` because (1) the user opted into wrap on their
