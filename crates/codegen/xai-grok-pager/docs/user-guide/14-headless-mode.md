...
zyth -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
zyth -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
zyth -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
zyth -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
zyth -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
zyth -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny sudo
zyth -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Allow all bash commands (auto-approve without prompting)
zyth -p "Build the project" --allow "Bash"
```
...
By default, each `zyth -p` invocation creates a fresh session. To maintain context across calls, use session flags.
...
zyth -p "Review the changes in this PR" --output-format json | jq -r '.sessionId'

# Continue in the same session
zyth -p "Now check for security issues" --resume "<id>"

# Optional: create with a client-chosen UUID (must not already exist)
zyth -p "hello" --session-id "$(uuidgen | tr '[:upper:]' '[:lower:]')" --output-format json
```
...
zyth -p "Remember: the secret number is 42" --output-format json
# Output includes "sessionId": "abc123"

# Resume that exact session
zyth -p "What's the secret number?" --resume abc123
```
...
```bash
zyth -p "Continue where we left off" -c
```
...
zyth -p "Generate a README" > README.md

# Parse JSON output with jq
zyth -p "List files" --output-format json | jq -r '.text'
```
...
zyth -p "Write a concise commit message for these changes:

$(git diff --staged)"

# Or read the prompt from a file
zyth --prompt-file ./prompt.txt
```
...
```bash
zyth -p "Review changes for bugs and security issues." \
...
```bash
zyth -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
zyth -p "Format all files" --yolo

# Run tests and fix failures
zyth -p "Run the tests and fix any failures" --cwd ~/projects/my-app --yolo
```
...
zyth -p "Run the test suite" --yolo
```
...
- **`zyth login --device-auth`** (or `--device-code`) — no browser needed on the target machine.
  See [Authentication > Device Code Flow](02-authentication.md#device-code-flow).
- **`zyth login`** — browser-based OAuth2 on machines with a GUI.
...
- Combine `--yolo` with `--rules` to set guardrails: `zyth -p "..." --yolo --rules "Never delete files"`.
...
- Resume: `zyth -p "continue" --resume "<id>"` or `zyth -p "continue" --continue`
