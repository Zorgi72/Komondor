...
zyth mcp list
zyth mcp list --json          # Machine-readable output
...
zyth mcp add filesystem -- npx -y @modelcontextprotocol/server-filesystem /path/to/dir

# Add a stdio server with environment variables (-e is repeatable)
zyth mcp add postgres -e DATABASE_URL=postgres://localhost/mydb -- npx -y @modelcontextprotocol/server-postgres

# Add a remote HTTP server
zyth mcp add --transport http sentry https://mcp.sentry.dev/mcp

# Add a remote server with an authentication header (--header is repeatable)
zyth mcp add --transport http api https://mcp.example.com/mcp --header "Authorization: Bearer YOUR_TOKEN"

# Add a remote SSE server
zyth mcp add --transport sse linear https://mcp.linear.app/sse

# Remove a server
zyth mcp remove github

# Diagnose a server's configuration and connectivity
zyth mcp doctor               # Check every configured server
zyth mcp doctor github        # Check one server
zyth mcp doctor --json        # Machine-readable output
```

The transport defaults to `stdio`; pass `--transport http` or `--transport sse` for remote servers.

By default `zyth mcp add` writes to `~/.grok/config.toml` (`--scope user`). Use `--scope project` to write to `.grok/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `zyth mcp list` shows servers from both scopes, marking project-scoped ones with `(project)`.

`zyth mcp remove` searches both scopes and exits 0 after removing the server. It exits 1 when the name is not found, or when the name is defined in both user and project scope — pass `--scope` to say which one to remove.
...
Use `zyth inspect` to see all loaded MCP servers and their sources:

```bash
zyth inspect          # Human-readable
zyth inspect --json   # Machine-readable
```
