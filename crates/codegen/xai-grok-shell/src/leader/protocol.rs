...
pub struct LeaderCapabilities {
    ...
    /// (driven by `zyth update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `zyth update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `zyth update` just installed; the leader uses
