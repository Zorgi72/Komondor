...
/// `zyth -r <remote_id>` in the same cwd, this function returns the already-restored
...
fn find_local_child_for_remote_in_root(
    ...
    // `zyth -r <remote_id>` before this fix was deployed.
...
/// worktree containing this session's cwd so `zyth worktree gc` expires by
...
mod find_local_child_tests {
    ...
    /// Regression: a second `zyth -r <remote_id>` must return the existing child
