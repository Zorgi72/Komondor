...
/// files (TUI tick vs `zyth login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `zyth setup`; runs retries to completion.
    Standard,
...
struct SyncOutcome {
    ...
    /// `zyth setup` doesn't misreport a lock skip as "no config".
    served: bool,
...
/// read-only `zyth setup --json` both build on this.
...
/// Post-login hook for `zyth login` and the ACP/TUI authenticate flow: clear any
...
/// Whether a credential exists that `zyth setup` could install config for.
...
/// `zyth setup`); ceasing to serve `fail_closed` rolls back.
...
/// Outcome of the `zyth setup` sync. The caller renders it — CLI presentation
...
/// Result of `zyth setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `zyth setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `zyth setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `zyth setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `zyth setup` would refuse
...
/// Run the `zyth setup` sync for the current principal. The caller must check
