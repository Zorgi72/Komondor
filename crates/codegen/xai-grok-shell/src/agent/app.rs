...
pub async fn run_stdio_agent(
    ...
    // Log the client that launched us (set by grok-desktop when spawning `zyth agent stdio`).
    ...
    let result = local_set
        .run_until(async move {
            ...
            // `zyth agent stdio` is a local/interactive entrypoint (spawned by
...
async fn run_headless_inner(
    ...
    register_fs_watch_runtime();
    xai_grok_telemetry::unified_log::set_version(xai_grok_version::VERSION);
    // `zyth agent [headless]` serves non-TUI automation; stamp proxy requests
    // as headless. IDE-facing `zyth agent stdio` stays interactive.
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a grok.com session. \
        Run `zyth login` to sign in, or use `zyth agent stdio` for API-key access.";
    ...
    let (mut auth, did_browser_flow) = if no_browser {
        ...
        match auth_manager.current() {
            ...
            None if auth_manager.is_expired() => {
                anyhow::bail!("Session expired. Please run 'zyth login' to re-authenticate.")
            }
            None => anyhow::bail!("No cached credentials found. Run `zyth login`."),
        }
...
///
/// * `relay_on_demand == false` (default — explicit `zyth agent leader`
...
pub async fn run_leader(
    ...
    local_set
        .run_until(async move {
            ...
            // `zyth agent leader` (devbox / systemd) has no local IPC clients
...
mod tests {
    ...
    /// `zyth agent leader` (devbox/systemd — no local IPC clients,
        ...
        local
            .run_until(async {
                let slot = spawn_leader_relay(
                    config,
                    false, // eager: explicit `zyth agent leader` invocation
                    demand_rx,
                    ws_to_agent_tx,
                    agent_to_ws_tx.clone(),
                    cancel.clone(),
